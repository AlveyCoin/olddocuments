# How to run a Alvey Lightning network node

Alvey has now its own implementation of the lightning network, it uses the eclair client. Currently, this is only supported on testnet, let's check how to set it up and run our very own lightning network client!. 

it is recommended to run **Debian 10 (buster)** as it has all the library versions we need

# Install Alvey Core

Let's go ahead and install Alvey on our Debian 10, the easy way is to use the Alvey Debian repository:

## Enable the Alvey repository
...
Before installing, we need to add the Alvey apt key
...
Alvey should be installed now, let's continue

## Create Alvey configuration file

We need to create the .alvey folder manually, also, we will create the configuration file inside of that folder, here's how:

`mkdir ~/.alvey`

`nano ~./alvey/alvey.conf` 
...
# Install Alvey Eclair - Lightning client

Let's install some dependencies to prepare for the lightning client:

## **Install JDK and Maven**

Alvey Eclair is developed using the Scala language. To run Alvey Eclair, you must first install the JDK, and OpenJDK 11 or above is recommended.
...
Now that we have every dependency installed, it's time to build the Eclair lightning client for Alvey. Again, inside our terminal we type:

- `git clone https://github.com/alveyproject/lightning-demo.git`
- `cd lightning-demo`
- `mvn install -DskipTests`

You should see something like this, if there's no errors, we can proceed with creating the configuration file and launch Alvey Eclair now.

## **Run Alvey Eclair**

Create and edit the configuration file ~/.alvey-eclair/eclair.conf as follows:
...
**PLEASE NOTE** if you modified the rpcuser and rpcpassword on alvey.conf, please modify this config file accordingly .

# Launch Alvey Eclair lightning client
...
Right-click the lower left corner of the local Alvey Eclair client and select “Copy URI” to get its node address (public key).
...
In the pop-up page, fill in the URI of the channel partner, and the capacity of the channel, which is set to 10 ALVEY as an example, this will be the maximum capacity for this channel, this means, you can only transact up to 10 ALVEY.
...
Click the “Channel” button in the upper left corner, select “Receive Payment”, fill in the receipt amount in the pop-up page, click the “Generate” button to get your own receipt address. Here we choose a very small amount of 1 Satoshi, which is equivalent to 0.00000001 ALVEY. Such a small transferred amount is unimaginable without using a lightning network.
...
Once closed, the remaining ALVEY in the channel will be refunded to the origin Alvey Core wallet.
