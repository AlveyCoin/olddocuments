...
这是 Alvey 量子链钱包的“**概览**”界面，也是启动之后的默认界面，可以看到下面这些信息：
...
在发送界面中，可以向 Alvey 地址发送 Alvey 。

![send](send.png)

**付给：** 在此处输入想要发送的Alvey接收地址，**请注意只有Alvey地址有效**。  
**标签：** 可选项，给上面输入的地址打标签。  
**金额：** 在此处输入要发送的Alvey数额。  

填写完各项后，点击发送，等待3秒后再次确认，即可将 Alvey 发送至目标地址。你也可以点击下方的添加收款人同时发送到多个地址。
...
在这里可以生成 Alvey 接收地址，并看到过往的接收地址。

![Receive](receive-1.png)

点击“请求付款”可以生成一个新的 Alvey 地址，同时也可以指定金额和备注。下图示例中填写了10个 Alvey 作为付款金额。
...
智能合约分为创建、发送和调用三个部分，下面以一个 ARC20 代币合约做说明。

合约代码如下使用[https://github.com/icodeface/ARC20Token](https://github.com/icodeface/ARC20Token)的 v2 版本，使用[Remix](https://remix.ethereum.org/)对合约代码进行编译，得到的 `Bytecode` 和 `ABI` 如下：
...
向合约发送数据需要填写合约地址和 ABI ，合约地址在上一个步骤得到， ABI 与之前的一致。填写好之后钱包会识别出来可以调用的方法，这里以 `transfer` 为例。注意 `Send To` 操作会消耗你的 Alvey。
...
和发送一样，填写好合约地址和 ABI ，钱包会识别出来可以本地调用的方法，本地调用无需消耗 Alvey。
...
在这里，可以发送、接收、添加 QR C代币，也可以通过开启 "log events" 查看ARC代币的交易记录。

![arc token](qrc.png)

#### 添加代币

点击 "Add Token" 可以添加一个新的 ARC 代币。
...
**Token Address**: 用于接收代币的 Alvey 地址，通过下拉选择一个地址。

填写完以上信息之后，点击 "Confirm" 即可添加一个ARC代币到你的钱包。同一个代币绑定同一个 Alvey 地址只能添加一次。

![add token](add-token.png)

#### 发送代币

在左侧选择一个代币，点击 "Send" 进 入发送界面，与发送Alvey有些相似。
...
填写完以上信息之后，点击 “Confirm" 即可发送ARC代币到指定地址。
...
在偏好设置->主要界面，勾选 "Enable log events" ，点击确认后钱包会重启并开启 "log events" ，此时即可查看 ARC 代币交易记录。
...
然后，然输入密码，**请务必保管好这个密码，否则你会丢掉你的Alvey**。
...
最新版本的 Alvey 钱包已经在界面上实现了备份管理。你可以轻松的备份、恢复你的钱包。点击 "File ->  Restore Wallet" ，在弹出的对话框可以看到有 4 种方式来恢复钱包，每种方式均可以选择一个备份文件来恢复钱包，若不选择将会使用现有的钱包目录下的 wallet.dat 文件。
...
在右侧选择要恢复的文件，然后点击 OK ，在弹出的框中点击 YES ，Alvey 钱包会自动启动，并使用你刚刚选择的备份文件恢复。
...
1. 钱包中有超过500个确认的 Alvey ；
2. 解锁钱包；
3. 保持钱包在线状态。

更多细节可以参考：[http://docs.alvey.site/zh/How-to-Stake-with-Alvey/](http://docs.alvey.site/zh/How-to-Stake-with-Alvey/)
...
Alvey Core钱包提供了在线的PoS共识，因此登录、配置该钱包之后就可以作为Super Staker进行操作、接受其他地址的委托。
...
Alvey Core钱包可以通过"设置"-"选项"-"允许Super Staking"设置来开启Super Staker模式，或者直接在命令行中使用`-superstaking=true`参数（示例为测试网环境）。
