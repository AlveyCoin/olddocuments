...
No hay otra URL para la billetera web de Alvey, por favor, tengan cuidado y verifiquen el candado verde que dice "Secure"  en el cual pueden corroborar el certificado de seguridad del sitio:
...
Bienvenidos a la documentacion de usuario para la billetera web de Alvey, a continuacion: 
...
* [How to receive and send ALVEY coins, sending with Ledger](#receive-and-send-alvey-coins)
...
The web wallet will has built-in capability for popular ARC20 tokens, and you can add additional tokens by entering the token's smart contract information. For example, if you want to add the XYZ token, find that token on the Explorer alveyscan.com, copy the contract _Address Hash_ field: 
