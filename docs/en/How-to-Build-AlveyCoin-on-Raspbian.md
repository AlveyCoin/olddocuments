# How To Build Alvey On Raspbian

Alvey is capable to run on small devices, it requires quite few CPU power, memory and storage to mine coins. But the binaries released by Alvey official are not suitable for Raspberry Pi Zero or First Generations. Here is a tutorial on how to build Alvey on a raspi. In this tutorial, we assume that you are using a raspbian system.

## System Requirements

Generally, a raspi is capable to build alvey, but we need:
...
Alvey requires quite few memory to run. However it is not true for building Alvey. It requires over 1.5G memory to build, which is much more than a raspi has. So we have to set the swap space to 2G:
...
Then we should switch to a stable release tag, for now the latest release for Alvey is `mainnet-ignition-v0.17.5`, but you should always build a latest one.
...
Boom! Now you have Alvey installed on your raspi!
