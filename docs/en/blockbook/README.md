# How to Set up blockbook for Alvey
...
2. Blockbook, along with alvey-core, requires at least 4GB ram to function, 8GB and higher is recommended (our deployment server has 3GB ram plus a 5GB swap file)
...
Blockbook creates several ports for Alvey when running, those ports are: 
...
Blockbook has a web UI with information about the alvey blockchain, just like on https://blockbook.alveyscan.com

SSL cert needed, if we don't have one, we can just create one with letsencrypt but we won't go into details in this doc.

Here's the Nginx config I'm using for blockbook.alveyscan.com


    server {
        listen 80;
        listen 443 ssl http2;
          ssl_certificate /etc/letsencrypt/live/blockbook.alveyscan.com/fullchain.pem; # managed by Certbot
        ssl_certificate_key /etc/letsencrypt/live/blockbook.alveyscan.com/privkey.pem; # managed by Certbot
        include /etc/letsencrypt/options-ssl-nginx.conf; # managed by Certbot
        ssl_dhparam /etc/letsencrypt/ssl-dhparams.pem; # managed by Certbot
        
        server_name blockbook.alveyscan.com;
    
    # force https-redirects
    if ($scheme = http) {
        return 301 https://blockbook.alveyscan.com$request_uri;
    }
