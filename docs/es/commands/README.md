...
La billetera de Alvey Core tiene una rica lista comandos, los cuales otorgan un control avanzado y completo de la billetera y transacciones de blockchain. Hay dos grupos de comandos que pueden ser usados con las billeteras de Alvey Core:
...
Este proceso manual se enfoca en los comandos de consola, los cuales se ingresan en una billetera que esta funcionando y pueden ser enviados usando RPC (Remote Procedure Calls) o en la linea de comandos a la billetera de servidor de alveyd o ingresados en la linea de comando al servidor alveyd o en la billetera de escritorio con interfaz grafica **alvey-qt** por medio de la ventana de depuracion de consola (ver Imagen 1).

![2019-1 Mac Debug Window with Prompt](https://i.imgur.com/K0uCLVr.jpg)
Imagen 1. La Consola de alvey-qt
...
Para la billetera servidor alveyd, los comandos de consola se ingresan utilizando la interfaz de aplicacion alvey-cli (ver Imagen 2).
...
Los Comandos de Consola se le ingresan a una billetera activa de Alvey Core y provee informacion adicional y control. Estos comandos son necesarios para operar la billetera de servidor alveyd, ya que no usa interfaz grafica.
...
See the startup commands on the alvey-qt wallet with Help ñ Command line options:

![2019-4 Startup Commands Win](https://i.imgur.com/CryGF13.jpg)
Figure 4. Startup commands

and on the command line itself with ìalveyd -?î:
...
Alvey-qt returns ìnullî after each (see Figure 6).
...
The destination can be a filename or path with a filename. The wallet must be fully decrypted (not just for staking only) for this command to work. alvey-qt will returns ìnullî, alveyd returns nothing. On Windows:
...
Clear all banned nodes IPs. Alvey-qt returns ìnullî, alveyd gives no response.
...
Disconnects a peer (node) using either the IP address or node number. Alvey-qt will return ìnullî, alveyd will return nothing.
...
Returns information about the blockchain. Here ìblocksî equals ìheadersî, so this wallet is synced to the latest blocks. ìmoneysupplyî gives the total ALVEY created (genesis blocks + all block rewards). ìsize_on_diskî gives the local storage size for blocks.
...
Hidden command. DEPERCATED. Command line interfaces (but not the alvey-qt GUI wallet) may invoke this command using `-getinfo`. The wallet below has a computer clock in sync with Alvey network time (timeoffset = 0), has 8 peer connections, and is unlocked for a long time (ìunlocked untilî is Unix epoch time in seconds).
...
* ìweightî gives the wallet weight in Satoshis, move the decimal point eight digits to the left to give these weights in units, here wallet weight is 148 ALVEY and the network weight is 12.23 million
...
Get the unconfirmed balance for the wallet, which is the amount in transactions received by the wallet that havenít yet been published in blocks. Here the wallet has received 7.0 ALVEY that haven't been confirmed in a block:
...
* "paytxfee" ñ the transaction fee in ALVEY per 1,000 bytes
...
Adds a WIF private key to your wallet, for example as returned by dumpprivkey or from another Alvey wallet. The wallet must be unlocked and requires a new wallet backup afterwards. The wallet will rescan for a few minutes to add any balance from the new address, and return ìnullî if successful.
...
Adds a public key that can be watched as if it were in your wallet but cannot be used to spend. The public key is 66 characters hex, and can be obtained using the `validateaddress` command. After entering this command, the wallet will rescan for a few minutes, alvey-qt returns ìnullî and alveyd returns nothing. The wallet should be backed up after importing a public key.
...
Refills the key pool with new private keys, with a default size of 100. The normal size of the keypool is 1,000 and the wallet opportunistically fills the keypool as addresses are used, so this command should use a 1,100 size, etc., to be meaningful. The wallet must be unlocked. alvey-qt returns ìnullî, alveyd returns nothing.
...
Prioritizes a block with an earlier time for blocks at the same height. Used in hard fork situations. See also `invalidateblock`. alvey-qt returns ìnullî if successful, alveyd returns nothing if successful.
...
Accepts the transaction into mined blocks at a higher (or lower) priority. A higher fee is not actually paid, the algorithm for selecting transactions into a block (which is trying to maximize fees) considers the transaction as having paid a higher fee. Alvey-qt and alveyd return ìtrueî:
...
Deletes the specified transaction from the wallet. Meant for use with pruned wallets and as a companion to `importprunedfunds`. This will affect wallet balances. Alvey-cli returns ìnullî.
...
Writes the memory pool to disk in the mempool.dat file. Alvey-qt returns ìnullî, alveyd has no response.
...
DEPRECATED (use `sendtoaddress`). Send an amount to a Alvey address, requires the wallet to be unlocked. The ìaccountî is for notation, it does not control the source of UTXOs. If successful, returns the transaction ID.
...
Send to multiple addresses, requires the wallet to unlocked. Amounts are floating point in ALVEY. Donít put blank spaces betweeen the parameters. If successful, returns a single transaction ID containing all the outputs. Here we send to two different addresses:
...
Send to multiple addresses, requires the wallet to be unlocked. Amounts are floating point in ALVEY. Can send to duplicate addresses which `sendmany` canít do. If successful, returns a single transaction ID containing all the outputs. Here we send two transactions to the same address:
...
DEPRECATED. Assigns and account name to the given address. Alvey-qt will return ìnullî if successful.
...
Hidden command. Waits for all the asynchronous validation queues (for the blockchain, mempool, etc.) to complete. For use by developers. Alvey-qt returns ìnullî, alveyd returns nothing.
...
Hidden command. Waits for a new block and returns the blockhash and height. Returns the current block on timeout or exit. This command works on command line only (not with the alvey-qt GUI wallet) and is used for development. The timeout parameter is given in milliseconds and 0 indicates no timeout.
...
alvey-cli returns no response, but you can check with `getwalletinfo` for "unlocked_until": 0 and check the padlock icon on alvey-qt.
...
The alvey-qt wallet will show lock status with the padlock icon and the Console returns ìnullî. alvey-cli returns no status but you can check the unlock status with `getwalletinfo` for ìunlocked_untilî: <Unix timestamp>

### walletpassphrasechange "oldpassphrase" "newpassphrase"

Changes the wallet passphrase from "oldpassphrase" to "newpassphrase", alvey-qt returns ìnullî, alveyd returns nothing.
